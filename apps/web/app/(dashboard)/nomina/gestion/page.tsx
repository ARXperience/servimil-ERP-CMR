...
import KpiCard from "@/components/nomina-ui/KpiCard";
...
import { formatCop, formatPeriodoEs, toNumber, normTxt, labelCorte } from "@/lib/nomina-module/utils/format";
import type { Empleado, NominaFila } from "@/lib/nomina-module/types/nomina";
...
export default function GestionPage() {
  ...
  const { data: pList } = useApi<{ periodos: { periodo: string; corte: string }[] }>("/api/nomina-data?ns=nomina&action=lista");
  ...
  const url = periodoEfectivo
    ? `/api/nomina-data?ns=gestion&action=mes&periodo=${encodeURIComponent(periodoEfectivo)}`
    : null;
  ...
  async function ejecutarRetirar(emp: Empleado) {
    ...
    try {
      const res = await fetch("/api/nomina-data?ns=gestion&action=empleado-retirar", {
  ...
  async function ejecutarBorrar(emp: Empleado) {
    ...
    try {
      const res = await fetch("/api/nomina-data?ns=gestion&action=empleado-borrar", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: emp.ID_Empleado }),
      });
      const j = await res.json();
