...
import { PieDist, BarDist } from "@/components/nomina-ui/Charts";
import { formatCop } from "@/lib/nomina-module/utils/format";
import type { CreditoCalc, KpisCartera, BucketEstado, BucketMora, BucketPagaduria } from "@/lib/nomina-module/calc/creditFifo";
...
function Dashboard() {
  const { data, error, isLoading } = useApi<DashResp>("/api/nomina-data?ns=credito&action=dashboard");
...
function CarteraMora() {
  const { data, error, isLoading } = useApi<MoraResp>("/api/nomina-data?ns=credito&action=mora");
  if (error) return <ErrBox e={error as Error} />;
  return (
...
function ResumenPagaduria() {
  const { data, error, isLoading } = useApi<PagResp>("/api/nomina-data?ns=credito&action=pagaduria");
  if (error) return <ErrBox e={error as Error} />;
