...
import { formatCop, formatPeriodoEs, labelCorte } from "@/lib/nomina-module/utils/format";
...
export default function DesprendibleTab({ periodo, empleados, filas }: Props) {
  ...
  const url = empId && periodo
    ? `/api/nomina-data?ns=desprendible&action=generar&id_empleado=${encodeURIComponent(empId)}&periodo=${encodeURIComponent(periodo)}&corte=${encodeURIComponent(corte)}`
    : null;
