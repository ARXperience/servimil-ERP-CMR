...
import { formatCop, formatPeriodoEs, toNumber, labelCorte } from "@/lib/utils/format";
import type { NominaFila, Novedad, Bonificacion, Alerta } from "@/lib/types/nomina";
...
export default function NominaPage() {
  const { data: pData, error: pErr, isLoading: pLoading } =
    useApi<PeriodosResp>("/api/data?ns=nomina&action=lista");
  ...
  const url = periodo
    ? `/api/data?ns=nomina&action=periodo&periodo=${encodeURIComponent(periodo)}&corte=${encodeURIComponent(corte || "")}`
    : null;
  const { data, error, isLoading } = useApi<PeriodoResp>(url);

  // Tendencia mensual (filtrada por corte si hay).
  const urlTendencia = `/api/data?ns=nomina&action=tendencia${corte ? `&corte=${encodeURIComponent(corte)}` : ""}`;
