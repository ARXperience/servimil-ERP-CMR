...
import { matchAny } from "@/lib/utils/format";
...
export default function AsesoresTab() {
  const { data, error, isLoading } = useApi<Resp>("/api/data?ns=asesores&action=lista");
  ...
  function pedirRetirar(a: Asesor) {
    setConfirmPrompt({
      ...
      onConfirm: async () => {
        closeConfirm();
        const res = await fetch("/api/data?ns=asesores&action=retirar", {
  ...
  function pedirBorrar(a: Asesor) {
    setConfirmPrompt({
      ...
      onConfirm: async () => {
        closeConfirm();
        const res = await fetch("/api/data?ns=asesores&action=borrar", {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ id: a.ID_Asesor }),
        });
        if (!res.ok) return;
  ...
  function refresh() {
    ...
    fetch("/api/data?ns=asesores&action=lista&fresh=1", { cache: "no-store" });
  }
