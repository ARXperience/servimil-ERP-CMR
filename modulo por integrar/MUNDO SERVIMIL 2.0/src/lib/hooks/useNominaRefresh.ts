...
export function useNominaRefresh() {
  ...
  return async function refresh(periodo?: string, corte?: string) {
    ...
    targets.push("/api/data?ns=nomina&action=lista");
    targets.push("/api/data?ns=nomina&action=tendencia");
    if (corte) targets.push(`/api/data?ns=nomina&action=tendencia&corte=${encodeURIComponent(corte)}`);
    if (periodo) {
      targets.push(`/api/data?ns=gestion&action=mes&periodo=${encodeURIComponent(periodo)}`);
      if (corte) {
        targets.push(`/api/data?ns=nomina&action=periodo&periodo=${encodeURIComponent(periodo)}&corte=${encodeURIComponent(corte)}`);
      }
    }
    ...
    if (periodo) {
      const url = `/api/data?ns=gestion&action=mes&periodo=${encodeURIComponent(periodo)}&fresh=1`;
